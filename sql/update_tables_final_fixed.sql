-- SQL FINAL CORREGIDO - Tipos compatibles en UNION
...
-- Verificar tipo de user_id y convertir si es necesario
DO $$ 
BEGIN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'account_balances_user_id_fkey' 
        AND table_name = 'account_balances'
    ) THEN
        ALTER TABLE account_balances 
        ADD CONSTRAINT account_balances_user_id_fkey 
        FOREIGN KEY (user_id) REFERENCES profiles(id);
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'withdrawal_limits_user_id_fkey' 
        AND table_name = 'withdrawal_limits'
    ) THEN
        ALTER TABLE withdrawal_limits 
        ADD CONSTRAINT withdrawal_limits_user_id_fkey 
        FOREIGN KEY (user_id) REFERENCES profiles(id);
    END IF;
END $$;
...
-- =========================================
-- 10. CREAR VISTAS PARA EL CRM (CON TIPOS COMPATIBLES)
-- =========================================

-- Vista de solicitudes pendientes usando profiles (con conversión de tipos)
CREATE OR REPLACE VIEW pending_requests_all AS
SELECT 
    'deposit'::TEXT as request_type,
    d.id::TEXT as id,  -- Convertir UUID a TEXT
...
SELECT 
    'withdrawal'::TEXT as request_type,
    w.id::TEXT as id,  -- Convertir VARCHAR a TEXT si es necesario
    ...
    COALESCE(p.full_name, 
             CASE 
                 WHEN p.nombre IS NOT NULL AND p.apellido IS NOT NULL 
...
SELECT 
    'transfer'::TEXT as request_type,
    t.id::TEXT as id,  -- Convertir UUID a TEXT
...
BEGIN
    ...
    RAISE NOTICE '📊 Tablas actualizadas: deposits, withdrawals, transactions';
    RAISE NOTICE '🔒 Políticas RLS aplicadas';
    RAISE NOTICE '👁️ Vistas CRM creadas: pending_requests_all, user_transaction_stats';
    RAISE NOTICE '✨ Sistema de solicitudes con aprobación CRM listo para usar';
END $$;