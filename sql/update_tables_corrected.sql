-- SQL CORREGIDO para actualizar las tablas existentes con la estructura real
-- Este script modifica las tablas existentes sin perder datos

-- =========================================
-- 1. ACTUALIZAR TABLA deposits
-- =========================================
-- La tabla deposits usa 'email' en lugar de 'user_id'

-- Primero agregar user_id si no existe (para relacionar con la tabla users)
ALTER TABLE deposits 
ADD COLUMN IF NOT EXISTS user_id TEXT;

-- Llenar user_id basándose en el email (si existe la relación)
UPDATE deposits d
SET user_id = u.id
FROM users u
...
-- Agregar columnas adicionales necesarias para el flujo CRM
ALTER TABLE deposits
...
-- Actualizar el constraint de status si es necesario
DO $$ 
BEGIN
...
-- =========================================
-- Esta tabla ya tiene user_id, solo agregar columnas faltantes
...
-- =========================================
-- Esta tabla ya tiene user_id y estructura básica
...
-- Políticas para deposits (usando email o user_id)
...
SELECT 
    ...
    d.id,
    COALESCE(d.user_id, d.email) as user_identifier,
    d.amount,
    d.status,
    d.created_at,
...
FROM (
    SELECT 'deposit' as request_type, COALESCE(user_id, email) as user_identifier, amount, status FROM deposits
    UNION ALL
    SELECT 'withdrawal', user_id, amount, status FROM withdrawals
    UNION ALL
    SELECT 'transfer', user_id, amount, status FROM transactions WHERE type = 'transferir'
) all_transactions
